...
- StringUtil: A little utility currently just containing a number formatting function outputting `1 234 567 890` for example, and a unity color to hex string converter
...
There are 2 different kinds of references to WannaBeClasses: **strong** and **weak**. Anything that holds a **strong** reference also increments the reference counter of the WannaBeClass, and must decrement the reference counter when it loses the reference to the WannaBeClass. **Weak** references do not touch the reference counter at all, which also means that the referenced WannaBeClass may get deleted while a weak reference to it was still held. The reference turns null at that point, with an exception, see below.

Importantly for weak references, great care must be taken with them when a WannaBeClass also implements support for the pooling this system provides. References to them will not turn null, and could actually turn into references to what was meant to be entirely new class instances.
