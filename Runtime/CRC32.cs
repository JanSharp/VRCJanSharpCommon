...
// This is a heavily modified version of the CRC32 file, every function but the crc32_1byte function has been
// removed and only the 1 lookup table has been kept, as well as it being ported to C# from C++.

namespace JanSharp
{
    public static class CRC32
    {
        public static uint Compute(byte[] data, uint previousCrc32 = 0u)
        {
            uint crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
            foreach (byte b in data)
                crc = (crc >> 8) ^ Crc32Lookup[(crc & 0xffu) ^ (uint)b];
